    ...
    def learn(self):
        if (self._learnRate):
            ...
            for i in range(len(self._weights)):
                self._weights[i]*=1-self._forgetRate
                self._weights[i]+=self._learnRate*output*self._inputValues[i]
            self.__dict__['_bias']*=1-self._forgetRate
    ...
    def __init__(self,inputNumber,neuronNumber,activFunc,weights=None,learnRate=None, forgetRate=None,bias=None):
        ...
        if _weights:
            if _bias!=None:
                ...
                self.__dict__['_neurons']=[NeuronHebb(_weights[:inputNumber],activFunc,learnRate=self._learnRate,forgetRate=self._forgetRate,bias=-0.8*np.random.ranf()-0.1) for _ in range(neuronNumber)]
        else:
            if _bias!=None:
                self.__dict__['_neurons']=[NeuronHebb([(0.8*np.random.ranf()+0.1)*np.random.choice([-1.0,1.0]) for _ in range(inputNumber)],activFunc,learnRate=self._learnRate,forgetRate=self._forgetRate,bias=_bias) for _ in range(neuronNumber)]
            else:
                self.__dict__['_neurons']=[NeuronHebb([(0.8*np.random.ranf()+0.1)*np.random.choice([-1.0,1.0]) for _ in range(inputNumber)],activFunc,learnRate=self._learnRate,forgetRate=self._forgetRate,bias=-0.8*np.random.ranf()-0.1) for _ in range(neuronNumber)]
    def __repr__(self):
